        ...
        >
          ...
          {step === 'form' && (
            <motion.div initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }}>
              <div className="mb-8">
                <span className="text-[#00783e] font-black text-xs uppercase tracking-widest">Inquiry Form</span>
                <h4 className="text-3xl font-black text-[#00251b] mt-1">{itinerary.title}</h4>
              </div>
              ...
              <form onSubmit={(e) => { e.preventDefault(); const err = validateForm(); if(!err){ setStep('review'); } else { setError(err); } }} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-[#00783e] uppercase ml-2">Travel Date</label>
                    <input required name="date" type="date" value={formData.date} onChange={handleInputChange} className="w-full bg-slate-50 border-2 border-transparent rounded-2xl p-4 focus:border-[#00783e] outline-none text-[#00251b]" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-[#00783e] uppercase ml-2">Travelers</label>
                    <input required name="travelers" type="number" placeholder="Count" value={formData.travelers} onChange={handleInputChange} className="w-full bg-slate-50 border-2 border-transparent rounded-2xl p-4 focus:border-[#00783e] outline-none text-[#00251b]" />
                  </div>
                </div>
                ...
                <input required name="whatsappNumber" type="tel" placeholder="Your WhatsApp (+94...)" value={formData.whatsappNumber} onChange={handleInputChange} className="w-full bg-slate-50 border-2 border-transparent rounded-2xl p-4 focus:border-[#00783e] outline-none text-[#00251b]" />
                ...
                <button type="submit" className="w-full bg-[#00251b] text-white py-5 rounded-2xl font-black text-lg flex items-center justify-center gap-2 hover:bg-[#00783e] transition-all">
          ...
          {step === 'review' && (
            <motion.div initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }}>
              <div className="mb-6">
                ...
                <h4 className="text-2xl font-black text-[#00251b] mt-1">Review Details</h4>
              </div>

              <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl p-6 font-mono text-sm text-slate-600 mb-6">
                <p className="font-bold text-[#00251b] mb-2 underline">BOOKING SUMMARY</p>
                <p>REF: #{refCode}</p>
          ...
          {step === 'success' && (
            <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center py-10">
              ...
              <h4 className="text-3xl font-black text-[#00251b] mb-2">Almost Done!</h4>
